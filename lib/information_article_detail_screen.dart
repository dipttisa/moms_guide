...
class InformationArticleDetailScreen extends StatefulWidget {
  ...
  const InformationArticleDetailScreen({super.key, required this.article});
  ...
  Future<void> _toggleFavorite() async {
    try {
      ...
      if (mounted) {
        ...
        // Отправляем результат на предыдущий экран без закрытия
        Navigator.pop(context, {
          'articleId': widget.article['id'],
          'isFavorite': _isFavorite,
        });
        // Сразу возвращаемся на экран деталей
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => InformationArticleDetailScreen(article: widget.article),
          ),
        );
      }
    } catch (e) {
